import{_ as i,X as t,Z as o,$ as l,V as c,W as d,F as r,Y as p}from"./framework-2eee3422.js";const u={},n=t,s=o,g=r,m=l,a=p,v=c,b=d,h=a(`<h1 id="为-mac-配置不同的-git-账号" tabindex="-1"><a class="header-anchor" href="#为-mac-配置不同的-git-账号" aria-hidden="true">#</a> 为 Mac 配置不同的 Git 账号</h1><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>【注】全文的用户环境为 MacOS，一些文件的目录有所不同。</p><ul><li><p>检查配置</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: <span class="token function">git</span> config <span class="token parameter variable">--list</span>  <span class="token comment"># 加 --global 为全局配置查看</span>
...
User gmail
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li><li><p>测试连接</p><p>格式： <code>ssh -T git@&lt; config 里面的 Host 别名&gt;</code></p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># 全局默认的个人账户，若没有配置 config， 则直接 ssh -T git@gitlab.xxx.cn</span>
$: <span class="token function">ssh</span> <span class="token parameter variable">-T</span> git@gitlab.xxx.cn
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li><li><p>clone 使用</p><p>格式： <code>git@{Host别名}:{具体项目地址}.git</code></p><p>以 GitHub 为例，假设你需要使用 github_account1 账号克隆 example_user/example_repo 仓库，则可以在命令行中执行以下命令：</p><ul><li>SSH 下载：<code>git clone git@gmail.github.com:example_user/example_repo.git</code></li><li>HTTPS 下载：<code>git clone https://gmail.github.com/example_user/example_repo.git</code></li></ul><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># Host 别名为 &#39;gmail.github.com&#39;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li></ul><h2 id="准备" tabindex="-1"><a class="header-anchor" href="#准备" aria-hidden="true">#</a> 准备</h2>`,5),k={href:"https://git-scm.com/download",target:"_blank",rel:"noopener noreferrer"},f=a(`<p>安转完成后，验证是否安转成功：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: <span class="token function">git</span> <span class="token parameter variable">--version</span>
// 若出现类似以下提示, 则说明安装成功
...
User gmail
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们来看看配置的相关设置：</p><table><thead><tr><th>键</th><th>值</th><th>规则</th></tr></thead><tbody><tr><td>Host</td><td>主机</td><td>主机别名。如果你不知道，可以填写主机名，<strong>通用配置就填 &quot;*&quot;</strong>。</td></tr><tr><td><strong>Hostname</strong></td><td>主机名</td><td><strong>【必须准确无误】</strong> 这里填写对应 Git 仓库的公有地址。如果你不知道，可以在 Git 仓库下通过 SSH 方式克隆一个项目，一般会出现如: <code>git@github.com:vuejs/vue.git</code> ，在 <strong>git@</strong> <strong>后面的 <code>github.com</code> 即为主机名</strong>。</td></tr><tr><td>IdentityFile</td><td>身份文件</td><td><strong>【写绝对路径】</strong> 相应账号的私钥存放地址，如：<code>~/.ssh/id_rsa</code></td></tr><tr><td><strong>User</strong></td><td>用户</td><td>用户名。</td></tr></tbody></table><p>再来看开始的通用配置，可有可无，主要用于解决 <strong><code>Unable to reach a settlement: [diffie-hellman-group1-sha1, diffie-hellman-group-exchange-sha1]...</code></strong> 的问题。许多公司的 Git 仓库还在用老旧的 <code>diffie-hellman-group1-sha1</code> 和 <code>diffie-hellman-group-exchange-sha1</code> 密钥交换算法，但是 <strong>OpenSSH</strong> 在 6.7 版本之后默认不再采用以上算法，因此我们需要在<strong>相应主机</strong>下手动添加 <code>KexAlgorithms +diffie-hellman-group1-sha1</code> 。</p><p>【<strong>Tips</strong>】我这里给出的是默认全局添加，若是公司出现类似报错，需要在公司的账户尾部添加 该设置！</p><h2 id="测试连接" tabindex="-1"><a class="header-anchor" href="#测试连接" aria-hidden="true">#</a> 测试连接</h2><p>测试 Git 账户连接：如果单个账户情况一般就是 <code>ssh -T git@xxx主机名</code> ，如果配置了 config，那么可以这样测试：<code></code><strong>ssh -T git@{config 里面的 user}.{config 里面的 Hostname}</strong></p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>// 全局默认的个人账户
$: <span class="token function">ssh</span> <span class="token parameter variable">-T</span> git@gmail.github.com

// 额外设置的公司git账户<span class="token punctuation">(</span>同上文中 config 的设置<span class="token punctuation">)</span>
// <span class="token function">ssh</span> <span class="token parameter variable">-T</span> git@<span class="token punctuation">{</span>Host 别名<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ul><li><strong>「第一次连接」</strong> 会出现主机连接验证： <code>Are you sure you want to continue connecting (yes/no/[fingerprint])? </code> <strong>输入</strong> <strong><code>yes</code></strong> <strong>回车</strong>就行了。</li><li>若连接成功则会出现类似信息:</li></ul><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>Hi xxx！ You&#39;ve successfully authenticated.but GitHub does not provide shell acess
...
// 利用公司的账户进行下载
$: <span class="token function">git</span> clone git@<span class="token punctuation">{</span> Host 别名 <span class="token punctuation">}</span>:vuejs/vue.git
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>感谢你的耐心阅读，希望你看完是带着收获离开的！</p>`,58);function x(_,y){const e=g("ExternalLinkIcon");return v(),b("div",null,[h,n("p",null,[s("首先，先 "),n("a",k,[s("下载 Git"),m(e)]),s(" ，并按照提示，一路选择安转即可。")]),f])}const H=i(u,[["render",x],["__file","system02.html.vue"]]);export{H as default};
