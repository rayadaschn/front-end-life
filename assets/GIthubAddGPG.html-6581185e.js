import{_ as l,Z as t,a0 as c,F as d,a1 as p,X as r,Y as o,$ as u}from"./framework-5dd7fabc.js";const v={},n=t,s=c,m=d,a=p,i=u,b=r,g=o,k=i(`<p>出于参与开源项目或是保护自身开源项目的安全考虑，在 GIthub 分支保护中，有一项 GPG 签名的设置。</p><p>当然，这不是它诞生的主要目的。我们可以看看提交的 Commit 记录，发现就算是不一个账号，如果在本地的 <code>git config</code> 中设置相同的 user 信息，最终提交的用户就是一样的。这个在网上有一个很形象的比喻：你的同事获取到了你的 <code>git config</code> ，便可以假装你删库跑路了。哈哈哈哈</p><p>而签名就是可以证明，你是不是真正的代码提交者，所以可以用于可靠的代码审计和追踪了。（删库跑路也不行了）</p><h2 id="安装-gpg-生成秘钥" tabindex="-1"><a class="header-anchor" href="#安装-gpg-生成秘钥" aria-hidden="true">#</a> 安装 GPG 生成秘钥</h2><p>在 MacOS 下，我们利用 brew 包进行下载：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: brew <span class="token function">install</span> gpg
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul><li><p>添加子公钥:</p><p>GPG 提供了交互式添加子密钥的方法，<code>gpg --expert --edit-key [用户ID]</code> 进入交互式密钥编辑，使用 <code>addkey</code> 添加子密钥。</p></li></ul><h2 id="问题排查" tabindex="-1"><a class="header-anchor" href="#问题排查" aria-hidden="true">#</a> 问题排查</h2><ol><li>电脑崩溃重启后，提交 git commit -S 时，提示 <code>gpg failed to sign the data</code>。</li></ol><p>具体报错示例:</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>error: gpg failed to sign the data:
gpg: Note: database_open 13xxxx901 waiting <span class="token keyword">for</span> lock <span class="token punctuation">(</span>held by <span class="token number">2507</span><span class="token punctuation">)</span> <span class="token punctuation">..</span>.
gpg: Note: database_open 13xxxx901 waiting <span class="token keyword">for</span> lock <span class="token punctuation">(</span>held by <span class="token number">2507</span><span class="token punctuation">)</span> <span class="token punctuation">..</span>.
gpg: Note: database_open 13xxxx901 waiting <span class="token keyword">for</span> lock <span class="token punctuation">(</span>held by <span class="token number">2507</span><span class="token punctuation">)</span> <span class="token punctuation">..</span>.
gpg: Note: database_open 13xxxx901 waiting <span class="token keyword">for</span> lock <span class="token punctuation">(</span>held by <span class="token number">2507</span><span class="token punctuation">)</span> <span class="token punctuation">..</span>.
gpg: Note: database_open 13xxxx901 waiting <span class="token keyword">for</span> lock <span class="token punctuation">(</span>held by <span class="token number">2507</span><span class="token punctuation">)</span> <span class="token punctuation">..</span>.
gpg: keydb_search failed: Operation timed out
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这个错误是由于 Git 使用 GPG（GNU Privacy Guard）签名提交时，GPG 卡在等待数据库锁，最后超时导致失败。</p><p>原因是 GPG 正在尝试访问其密钥数据库。但数据库被另一个进程（PID 2507）锁住了，GPG 无法访问。最终操作超时，Git 无法完成提交。</p><p>解决办法，查找所有 GPG 相关锁文件, 并将其删除</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: <span class="token function">ls</span> <span class="token parameter variable">-l</span> ~/.gnupg/*.lock
$: <span class="token function">rm</span> <span class="token parameter variable">-f</span> ~/.gnupg/*.lock

<span class="token comment"># 查找其他 GPG 数据库锁文件</span>
$: <span class="token function">find</span> ~/.gnupg <span class="token parameter variable">-name</span> <span class="token string">&quot;*.lock&quot;</span>
<span class="token comment"># 然后删除</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="参考文档" tabindex="-1"><a class="header-anchor" href="#参考文档" aria-hidden="true">#</a> 参考文档</h2>`,36),f={href:"https://zhuanlan.zhihu.com/p/137801979",target:"_blank",rel:"noopener noreferrer"},G={href:"https://www.yangqi.show/posts/gpg-github",target:"_blank",rel:"noopener noreferrer"},_={href:"https://www.bitlogs.tech/2019/01/gpg%E4%BD%BF%E7%94%A8%E6%95%99%E7%A8%8B/",target:"_blank",rel:"noopener noreferrer"};function I(S,E){const e=m("ExternalLinkIcon");return b(),g("div",null,[k,n("p",null,[h,s("：在 GitHub 中添加 GPG 公钥。具体步骤参照 "),n("a",x,[s("这里"),a(e)])]),y,n("ul",null,[n("li",null,[n("a",f,[s("简明 GPG 概念"),a(e)])]),n("li",null,[n("a",G,[s("使用 GPG 为 GIthub 签名"),a(e)])]),n("li",null,[n("a",_,[s("GPG 教程"),a(e)])])])])}const D=l(v,[["render",I],["__file","GIthubAddGPG.html.vue"]]);export{D as default};
