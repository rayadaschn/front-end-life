...
$: <span class="token function">git</span> commit <span class="token parameter variable">-m</span> <span class="token punctuation">[</span>message<span class="token punctuation">]</span>
$: <span class="token function">git</span> push
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>​ 若你和同事的分支不同，此时并不是用 <code>git pull</code> 同步代码， 而是用</p><p>个人使用，已经写完代码了，用方案 1 推送；还未写完，则用方案 2 临时贮藏。</p><h3 id="版本回退" tabindex="-1"><a class="header-anchor" href="#版本回退" aria-hidden="true">#</a> 版本回退</h3><blockquote><p>版本回退需注意 <code>git revert</code> 和 <code>git reset</code> 的区别</p></blockquote><ul><li><code>git revert</code><strong>git revert</strong> 撤销某次操作，此次<strong>操作之前和之后的 commit 和 history 都会保留</strong>，并且把这次撤销作为一次最新的提交。git revert 是提交一个新的版本，将需要 revert 的版本的内容再反向修改回去，版本会递增，不影响之前提交的内容。</li></ul><ol><li>当代码已经 commit 但没有 push 时，可使用如下命令操作： <code>git revert HEAD //撤销倒数第一次提交</code><code>git revert HEAD^ //撤销倒数第二次提交</code><code>git-revert HEAD~2 //撤销倒数第三次提交</code><code>git revert commit //（比如：fa042ce57ebbxxxxxxxxxxx2c58ee7ff）撤销指定的版本，撤销也会作为一次提交进</code></li><li>当代码已经 commit 并 push 时，可使用如下命令： <code>git revert HEAD~1 //代码回退到前一个版本</code></li></ol><p>当回退有冲突时，需手动合并冲突并进行修改，再 commit 和 push。<strong>这相当于增加了一次新的提交并且版本库中有记录。</strong></p><ul><li><p><strong><code>git reset</code> 推荐!!!!</strong><strong>git reset</strong> 是撤销某次提交，但是<strong>此次之后的修改都会被退回到暂存区</strong>。除了默认的 mixed 模式，还有 soft 和 hard 模式。</p><blockquote><p><strong>--soft :</strong> 不删除工作空间改动代码，<strong>撤销 commit</strong>，<strong>不撤销 <code>git add . </code></strong><strong>--hard :</strong> 删除工作空间改动代码，<strong>撤销 commit</strong>，<strong>撤销<code>git add .</code></strong></p><pre><code>* 注意完成这个操作后，就恢复到了上一次的commit状态。
</code></pre><p><strong>--mixed :</strong> 【默认参数】不删除工作空间改动代码，<strong>撤销 commit</strong>，并且 **撤销 <code>git add .</code> **</p><p>简单的讲，正常提交是： <code>git add .</code> --&gt; <code>git commit</code></p><p>对应的回退版本是: <code>git reset --soft</code> --&gt; <code>git reset --mixed</code></p></blockquote></li></ul><ol start="3"><li>如果我们的有两次 commit 但是没有 push 代码</li></ol><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: <span class="token function">git</span> reset HEAD~1      //撤销前一次 commit，所有代码回到 Working Copy
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>注意！当我们使用强制指令时，若在远程的该分支中有他人的贡献，<code>--force</code> 是会覆盖掉他人的代码的，所以为了保险起见，应当用 <code>--force-with-lease</code> 。</p></blockquote><ol start="5"><li>只回退某个指定文件到指定版本</li></ol><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: <span class="token function">git</span> reset a4e215234aa4927c85693dca7b68e9976948a35e  xxx
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="参考文档" tabindex="-1"><a class="header-anchor" href="#参考文档" aria-hidden="true">#</a> 参考文档</h2>`,3),$={href:"https://jdf2e.github.io/jdc_fe_guide/docs/git/branch",target:"_blank",rel:"noopener noreferrer"},_={href:"https://www.runoob.com/git/git-basic-operations.html",target:"_blank",rel:"noopener noreferrer"},q={href:"https://godbasin.github.io/2019/11/10/change-log/",target:"_blank",rel:"noopener noreferrer"},G={href:"https://www.ruanyifeng.com/blog/2012/07/git.html",target:"_blank",rel:"noopener noreferrer"},y={href:"https://segmentfault.com/q/1010000002477106",target:"_blank",rel:"noopener noreferrer"};function C(w,B){const a=o("ExternalLinkIcon");return l(),c("div",null,[d,n("h2",p,[u,s(),n("a",m,[s("常用的 GIT 操作指令"),e(a)])]),v,n("h2",g,[b,s(" 利用 "),n("a",h,[s("git-gz"),e(a)]),s(" 规范代码提交")]),k,n("ul",null,[n("li",null,[s("自动生成 "),n("strong",null,[n("a",f,[s("CHANGELOG–conventional-changelog-cli"),e(a)])])])]),x,n("ul",null,[n("li",null,[n("a",$,[s(" JDC 前端代码规范 (jdf2e.github.io)"),e(a)])]),n("li",null,[n("a",_,[s("Git 基本操作 "),e(a)])]),n("li",null,[n("a",q,[s("前端 CHANGELOG 生成指南"),e(a)])]),n("li",null,[n("a",G,[s("Git 分支管理"),e(a)])]),n("li",null,[n("a",y,[s("Git --no--ff"),e(a)])])])])}const N=t(r,[["render",C],["__file","GitOperationManuals.html.vue"]]);export{N as default};
