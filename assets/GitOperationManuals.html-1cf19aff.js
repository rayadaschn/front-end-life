...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><ul><li><code>git config</code>：Git 命令行工具的配置命令。</li><li><code>--global</code>：表示该配置是全局性质的，会应用在你的所有代码仓库中。</li><li><code>http.https://github.com.proxy</code> ：作为配置项名称，它表示对应的是使用 HTTPS 协议访问 GitHub 并需要进行代理配置。</li><li><code>socks5://127.0.0.1:1086</code>：代理服务器的地址和端口号。其中，socks5 表示使用 SOCKS5 协议进行代理，<code>127.0.0.1</code> 指的是代理服务器的 IP 地址或主机名，1086 则是端口号。你需要根据自己实际的代理情况进行相应的修改，并保证对应的代理能够正常工作。</li></ul><p>取消代理：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># 取消全局代理</span>
