import{_ as l,V as t,W as d,X as n,Y as e,Z as a,$ as i,F as c}from"./framework-2060dede.js";const r={},p=i(`<h1 id="github添加-gpg-签名" tabindex="-1"><a class="header-anchor" href="#github添加-gpg-签名" aria-hidden="true">#</a> GIthub添加 GPG 签名</h1><p>出于参与开源项目或是保护自身开源项目的安全考虑，在 GIthub 分支保护中，有一项 GPG 签名的设置。</p><p>当然，这不是它诞生的主要目的。我们可以看看提交的 Commit 记录，发现就算是不一个账号，如果在本地的 <code>git config</code> 中设置相同的 user 信息，最终提交的用户就是一样的。这个在网上有一个很形象的比喻：你的同事获取到了你的 <code>git config</code> ，便可以假装你删库跑路了。哈哈哈哈</p><p>而签名就是可以证明，你是不是真正的代码提交者，所以可以用于可靠的代码审计和追踪了。（删库跑路也不行了）</p><h2 id="安装-gpg-生成秘钥" tabindex="-1"><a class="header-anchor" href="#安装-gpg-生成秘钥" aria-hidden="true">#</a> 安装 GPG 生成秘钥</h2><p>在 MacOS 下，我们利用 brew 包进行下载：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: brew <span class="token function">install</span> gpg  
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>列出私钥：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>$: gpg --list-secret-keys 
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><ul><li><p>添加子公钥:</p><p>GPG 提供了交互式添加子密钥的方法，<code>gpg --expert --edit-key [用户ID]</code> 进入交互式密钥编辑，使用 <code>addkey</code> 添加子密钥。</p></li></ul><h2 id="参考文档" tabindex="-1"><a class="header-anchor" href="#参考文档" aria-hidden="true">#</a> 参考文档</h2>`,27),m={href:"https://zhuanlan.zhihu.com/p/137801979",target:"_blank",rel:"noopener noreferrer"},b={href:"https://www.yangqi.show/posts/gpg-github",target:"_blank",rel:"noopener noreferrer"},g={href:"https://www.bitlogs.tech/2019/01/gpg%E4%BD%BF%E7%94%A8%E6%95%99%E7%A8%8B/",target:"_blank",rel:"noopener noreferrer"};function k(h,x){const s=c("ExternalLinkIcon");return t(),d("div",null,[p,n("p",null,[o,e("：在 GitHub 中添加 GPG 公钥。具体步骤参照 "),n("a",u,[e("这里"),a(s)])]),v,n("ul",null,[n("li",null,[n("a",m,[e("简明 GPG 概念"),a(s)])]),n("li",null,[n("a",b,[e("使用 GPG 为 GIthub 签名"),a(s)])]),n("li",null,[n("a",g,[e("GPG 教程"),a(s)])])])])}const f=l(r,[["render",k],["__file","GIthub添加GPG 签名.html.vue"]]);export{f as default};
