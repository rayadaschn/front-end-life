...
2. 后面俩个 Promise.resolve() 会依次进入微任务队列, 依次打印 `5` 和 `7 8`。此时 p2 状态完成，p1 的 await 吸收 p2 的结果，但还需要进入微任务等待一次。
3. 最后俩个 Promise.resolve() 会依次进入微任务队列, 依次打印 `6` 和 `undefined 9`。此时 p1 的 await 状态完成，打印 `2`。
