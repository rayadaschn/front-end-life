...
export default {
  props: ['modelValue'],
  emits: ['update:modelValue'], // 额外声明，默认绑定的emit更新变为 update:modelValue
  methods: {
    updateValue(newVal) {
...
- slot 插槽: 父组件向子组件传递内容
  ...
  - 作用域插槽 `v-slot="{ item }"` : 父组件写结构，子组件提供数据

    ```vue
    <!-- 父组件 -->
    <Child v-slot="slotProps">父组件使用这个数据{{ slotProps.msg }}</Child>
...
- keep-alive
  ...
  - 同 `v-show` 的区别:
    - `keep-alive` 是在 vue 框架层级进行的 JS 对象渲染;
    - `v-show` 是 CSS display 控制显示和隐藏
...
可以用计算属性过滤 `v-for` 循环的结果，而不是在 `v-for` 指令中使用条件语句。
...
- `history.pushState()`、`history.back()` 和 `history.replaceState()` 改变 URL，但不会触发跳转
- H5 history 需要后端支持
...
- 更好的 ts 支持;
- 更好的代码组织，更好的逻辑抽离；
- Vue2 中所有属性都是通过 this 访问，this 存在指向明确问题；

### Vue3 生命周期

生命周期其实都是同步进行的，区别在于调用时机不同。
