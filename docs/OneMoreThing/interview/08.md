...
## 原型链怎么工作的？class 本质是什么？

JavaScript 是基于原型的语言，每个对象都有一个内部 `[[Prototype]]` 指针指向其原型对象。通过 new 创建实例时，会将实例的 `[[Prototype]]` 指向构造函数的 `prototype`。当访问属性时，会沿着原型链向上查找，直到 `Object.prototype`，最终为 `null`。

class 本质是构造函数的语法糖，方法定义在 `prototype` 上，`extends` 实现的是原型链继承，同时构造函数之间也存在 `proto` 继承关系。

## 闭包的使用场景？

闭包是指函数可以访问其定义时所在作用域的变量，即使函数在该作用域之外执行。
...
有两个，一个是 node 的一个是 Vue 的。

1. `process.nextTick` 是 Node 特有的微任务机制，它的回调会在当前调用栈清空后、进入事件循环前执行，**优先级高于 Promise 微任务**，因此不能滥用，否则会阻塞 I/O。
2. Vue 的 nextTick 本质是利用微任务（Promise.then）在当前同步代码执行完后统一执行回调。
   因为 Vue 的 DOM 更新是异步批量更新的，所以需要 nextTick 在 DOM 更新完成后执行逻辑。内部维护一个回调队列，使用 Promise 微任务在本轮事件循环末尾 flush 队列。
