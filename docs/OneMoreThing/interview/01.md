...
![offsetWidth 定义](https://cdn.jsdelivr.net/gh/rayadaschn/blogImage@master/img/202503291332615.png)

实际上是看 `box-sizing`(计算 width 值的方式)，默认为 `content-box`，可改为 `border-box`。
...
双飞翼布局中，需要多用一个 div 盒子包裹主体内容，使其具有延展性。同时也用到了`margin` 负值将自身宽度给抵消了。左侧是`margin-left: -100%`是整个行块宽度，右侧是`margin-left: -200px`是自身宽度。
...
`attribute` 是 HTML 标签的静态属性，`property` 是 DOM 对象的动态属性。

在 HTML 元素中，`attribute`是在 HTML 标签中定义的，比如 id、class、href 等，这些都是写在标签里的属性。

而`property`则是当浏览器解析 HTML 后，生成的 DOM 对象中的属性，比如`element.href`。这时候，**有些`attribute`和`property`是同步的，比如 id，但有些则不同步，比如`checked`这个属性**。例如，在 HTML 中，`checked`作为一个`attribute`存在时，它的值只是初始值，而`property`中的`checked`是一个布尔值，会根据用户的操作改变。

因此，需要注意，修改`attribute`并不会直接影响`property`。

在 Web 开发中，**Property** 和 **Attribute** 是容易混淆但本质不同的概念，主要与 HTML 元素和 DOM（文档对象模型）相关。以下是它们的核心区别和联系：
...
| **特征**     | **Attribute**                 | **Property**                       |
| ------------ | ----------------------------- | ---------------------------------- |
| **定义位置** | 存在于 **HTML 标签**中        | 存在于 **DOM 对象**中              |
| **数据类型** | 始终是**字符串**              | 可以是**任意类型**（布尔、数字等） |
| **同步性**   | 初始值（HTML 解析时设置）     | 动态值（可能随用户操作改变）       |
| **访问方式** | `element.getAttribute("xxx")` | `element.xxx` 或 `element["xxx"]`  |
| **自定义性** | 可自定义（如 `data-*`）       | 由 DOM 规范定义，不可自定义        |
...
- listener: 事件触发处理函数;
- 第三个参数 useCapture 是一个布尔值, 表示事件是否在捕获阶段触发处理函数。默认为 `false`，事件在冒泡阶段触发监听器。

第三个参数还可以是一个对象:

```js
el.addEventListener(type, function, {
  capture: false, // 事件是否在捕获阶段触发处理函数, 等效于 useCapture
  once: false, // 监听器不会自动移除，会持续响应事件（若设为 true，则只触发一次后移除）。
  passive: false // 表示是否将事件处理程序设置为被动模式。被动模式的事件处理程序永远不会调用 event.preventDefault()，因为浏览器会在执行事件处理程序之前假设它不会调用阻止默认行为的方法，从而优化页面的滚动性能。
})
```
...
3. 使用 VH 和 VW 单位, VH 和 VW 是视口高度和视口宽度的单位，可以用来实现更灵活的布局。
