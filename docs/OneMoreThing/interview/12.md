...
Webpack 代码分割主要有三种方式：多入口 `entry`、动态 `import` 以及 `SplitChunksPlugin`。

实际项目中主要使用动态 `import` 实现路由级懒加载，并通过 `splitChunks` 抽离公共模块和第三方依赖。

底层实现是 webpack 在构建阶段生成多个 chunk，运行时通过 script 动态加载。

合理的代码分割可以减少首屏体积，提高缓存命中率，但不能拆得过碎，否则会增加请求开销。
